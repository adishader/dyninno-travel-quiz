...
3. **Quiz flow** — 15 multiple-choice questions, one at a time, single correct answer per question (single-select, not multi-select).
   ...
   - A **Submit answer button** is present on each question. It is **disabled by default** (not clickable, distinct visual style from its enabled state) until the participant selects an option. Once an option is selected, the button becomes enabled/clickable (switches to its active visual style). **Default button label**: key `quiz.submit_button_default` in the translations file (currently "Submit" / "Enviar" as a **placeholder** — real copy should be confirmed against the Figma Question page design, not treated as final).
   - On submitting the selected answer: lock the choice, immediately show whether it was correct or incorrect (highlight the correct answer if the user chose wrong). The **Submit button itself becomes the feedback element**: its text changes to a random message from the matching pool below, its background color changes accordingly, and it becomes non-clickable for the remainder of that question.
     - **Correct answer** — background `#66B11B`, button text picked randomly from translations keys `feedback.correct.1`–`feedback.correct.5` (EN: "Correct! 🥳" / "Just wow! 👏" / "Amazing! 🤩" / "Nailed it! 💅" / "You are on fire! 💃")
     - **Wrong answer** — background `#EBEEF5`, button text picked randomly from translations keys `feedback.wrong.1`–`feedback.wrong.5` (EN: "Now you know... 🤷🏻‍♀️" / "So close! 👀" / "We're always learning 🧐" / "Hmmm 🤔" / "Oops 🙈")
